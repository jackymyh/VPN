...
public class Authentication {
	...
	public Authentication(InstanceType instanceType, String ipAddress, int portNumber, String sharedSecret) {
	...
	public byte[] unsign(byte[] msg) throws Exception {
		// msg = "Hello World".getBytes("UTF-8");
		// Read the signature from the signedMessage (and its length)
		System.out.println("Unsigning msg..."+msg+" msg.length "+msg.length);
        int length = byteArrayToInt(Arrays.copyOf(msg,4));
        length = 0;
        System.out.println("Unsigning length..."+length);
        byte[] sentSignature = Arrays.copyOfRange(msg,4,4+length);
        //System.out.println("Unsigning 3...");
